/* ============================================================================
 *  Tabu-Novelty Search — CPU PURE (Portable, Cache-Optimized)
 *  Compilación: gcc -O3 -std=c99 -march=native -ffast-math -o tns_cpu tns_cpu_pure.c -lm
 * ============================================================================ */
...
/* -------------------------------------------------------------------------- */
/*  xoshiro256+ — PRNG de 64 bits, ~2× más rápido que rand()                  */
/* -------------------------------------------------------------------------- */
...
{
    return (xoshiro256plus() >> 11) * (1.0 / (1ULL << 53));
}

static inline double randf_sym(void)
{
    return randf() * 2.0 - 1.0;
}

/* -------------------------------------------------------------------------- */
/*  Estructuras (layout natural, sin packed, alineación explícita en arrays)  */
/* -------------------------------------------------------------------------- */
typedef struct {
    uint64_t signature;
...
static inline __attribute__((always_inline, hot))
int hamming_dist(uint64_t a, uint64_t b)
{
    return __builtin_popcountll(a ^ b);
}

/* FLE: Collatz trajectory anomaly scorer */
static inline __attribute__((always_inline, hot))
...
/* -------------------------------------------------------------------------- */
/*  TSC: Phase Hijack (CPU)                                                   */
/* -------------------------------------------------------------------------- */
static __attribute__((hot)) void tsc_hijack(const double *restrict champion_state,
                                             double blocked_v)
{
    ...
    for (int i = 0; i < POPULATION_SIZE; i++) {
        ...
        for (int trial = 0; trial < TRIALS_PER_INDIVIDUAL; trial++) {
            ...
            if (limit > 4)
                __builtin_prefetch(&tabu_memory[(trial + 1) % limit], 0, 3);
            ...
            for (int m = 0; m < limit; m++) {
                int d = hamming_dist(sig, tabu_memory[m].signature);
                if (d < dmin) dmin = d;
            }
...
/* Insertion sort (óptimo para N ≤ 64) */
...
int main(void)
{
    /* Seed xoshiro256+ con time + splitmix64 */
    ...
    printf("=== Tabu-Novelty Search | CPU PURE ===\n");
    printf("ARCH    : Portable C99 | xoshiro256+ | Cache-optimized\n");
    printf("VECTOR  : %d dims | POP: %d | MEM: %d | STAG: %d | ERAS: %d\n",
           VECTOR_DIM, POPULATION_SIZE, MEMORY_SIZE, STAGNATION_LIMIT, MAX_ERAS);
    printf("METRICA : Hamming 64-bit (__builtin_popcountll)\n");
    printf("-----------------------------------------------------------\n\n");
